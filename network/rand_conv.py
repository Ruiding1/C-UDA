...
class RandConvModule(nn.Module):
    def __init__(self, net=None, kernel_size=3, in_channels=3, out_channels=3,
        ...
        # Add RandConv layer with kernel size [1, 3, 5, 7], output channel 3
        self.randconv = MultiScaleRandConv2d_Ours(in_channels=in_channels, out_channels=out_channels, kernel_sizes=kernel_size, stride=1)
...
class RandConv2d_OUR(Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, **kwargs):
        ...
        :param kwargs:
        """
        super(RandConv2d_OUR, self).__init__(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size, bias=rand_bias, **kwargs)
    ...
    def forward(self, input):
        output = super(RandConv2d_OUR, self).forward(input)
...
class MultiScaleRandConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes,
        ...
        self.multiscale_rand_convs = nn.ModuleDict(
            {str(kernel_size): nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=kernel_size // 2, bias=False) for kernel_size in kernel_sizes})
...
class MultiScaleRandConv2d_Ours(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes, stride, **kwargs ):
        """
        ...
        :param bias:
        """
        super(MultiScaleRandConv2d_Ours, self).__init__()
    ...
    def randomize(self):
        self.current_scale = str(self.scales[random.randint(0, self.n_scales-1)])
        self.multiscale_rand_convs[self.current_scale]


    def forward(self, input):
        output = self.multiscale_rand_convs[self.current_scale](input)
        # print(self.multiscale_rand_convs[self.current_scale])
        return output
